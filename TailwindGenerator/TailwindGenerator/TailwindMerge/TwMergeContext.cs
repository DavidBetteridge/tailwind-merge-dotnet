...
internal partial class TwMergeContext
{
    ...
    {
        var classNameParts = className.Split( Constants.ClassNameSeparator[0] );
    ...
    {
        _config.ConflictingClassGroups.TryGetValue( classGroupId, out var conflicts );
    ...
    internal ClassModifiersInfo SplitModifiers( string className )
    {
        ...
        for( var i = 0; i < className.Length; i++ )
        {
            ...
            if( bracketDepth == 0 )
            {
                ...
                {
                    modifiers.Add( className.Substring(modifierStart,i-modifierStart) );
    ...
    private string? GetClassGroupIdRecursive( string[] classNameParts, ClassNameNode node )
    {
        ...
        {
            var classGroupId = GetClassGroupIdRecursive( classNameParts.Skip(1).ToArray(), nextNode );
    ...
    private static Regex ArbitraryPropertyRegex() =>
